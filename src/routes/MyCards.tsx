import { useEffect, useState } from "react";
import defaultImage from "../assets/user-regular-new.png";
import { Link } from "react-router-dom";
import { myCardByApi } from "../services/cards";
import { CardType } from "../@types/types";

const MyCards = () => {
  const [cards, setCards] = useState<CardType>();
  const [error, setError] = useState<any>();

  useEffect(() => {
    userCards();
  }, []);

  const userCards = async () => {
    try {
      const response = await myCardByApi();

      setCards(response.data);
      setError(null);
    } catch (error) {
      if (error.response && error.response.status === 401) {
        setError("Error: Please Login");
      } else {
        setError("An error occurred.");
      }
      console.error("Error Cards:", error);
    }
  };
  return (
    <div className="flex flex-col">
      <h1 className="text-3xl text-center mb-4">My Cards</h1>
      <div className="flex flex-wrap justify-center items-center gap-5 mt-2 dark:text-white">
        {cards?.map((c) => (
          <div className="flex flex-col justify-center items-center p-5 rounded-md text-center bg-gray-200 m-2 dark:bg-slate-500 dark:text-white" key={c._id}>
            <Link to={`/cards/${c._id}`}>
              <img
                src={c.image ? c.image.url : defaultImage}
                alt={c.image ? c.image.alt : "No image available"}
                className="w-72 h-48 object-cover mt-3 rounded-md shadow-md"
                onError={(e) => {
                  const target = e.target as HTMLImageElement;
                  target.src = defaultImage;
                }}
              />
              <h2 style={{ color: "hotpink" }} className="text-l px-1 py-0.5 font-bold">{c.title}</h2>
              <div className=" bg-stone-300 m-2 dark:bg-slate-400 p-5 rounded-md drop-shadow-md  dark:text-white w-80 text-left">
                <h3 style={{ color: "hotpink" }} className="text-l  py-0.5 font-bold">Address :</h3>
                <p className="text-sm">City : {c.address.city}, Street : {c.address.street}</p>
                <p className="text-sm">State : {c.address.state}, Zip :  {c.address.zip}</p>
                <p className="text-sm">Country : {c.address.country}</p>
              </div>
            </Link>
          </div>
        ))}
      </div>
    </div>
  );
};

export default MyCards;